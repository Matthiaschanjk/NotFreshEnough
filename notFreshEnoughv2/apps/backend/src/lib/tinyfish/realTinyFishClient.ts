...
import type { FetchedWebSurface } from "./fetchers/web";
import type { TinyFishClient } from "./client";

type FindingSignal = TinyFishFinding["signal"];

interface TinyFishAutomationResponse {
  result?: unknown;
  data?: unknown;
  output?: unknown;
...
function pickFirstString(value: unknown, candidates: string[]): string | undefined {
  if (!value || typeof value !== "object") {
    return undefined;
  }
  ...
  for (const candidate of candidates) {
    const matched = record[candidate];
...
function pickHeadings(value: unknown): string[] {
  if (!value || typeof value !== "object") {
    return [];
  }

  const record = value as Record<string, unknown>;
  const headingSource = record.headings ?? record.sections ?? record.outline;
  if (!Array.isArray(headingSource)) {
    return [];
  }
  ...
  return headingSource
    .map((entry) => {
      ...
      if (entry && typeof entry === "object") {
        return pickFirstString(entry, ["title", "heading", "name"]);
      }
...
function extractTinyFishPage(url: string, payload: TinyFishAutomationResponse): FetchedWebSurface {
  const source =
    payload.result ??
    payload.data ??
    payload.output ??
  ...
  const title =
    pickFirstString(source, ["title", "page_title", "name"]) ??
    pickFirstString(payload, ["title", "page_title"]) ??
    url;

  const text =
    pickFirstString(source, ["content", "text", "markdown", "body", "summary"]) ??
    toText(source) ??
    "";

  const squashedText = text.replace(/\s+/g, " ").trim();

  return {
    title,
    url,
    text: truncateText(squashedText, 5000),
    snippet: truncateText(squashedText, 420),
    headings: pickHeadings(source)
  };
}
...
export class RealTinyFishClient implements TinyFishClient {
  ...
  async investigateProject(input: JudgeProjectInput): Promise<TinyFishInvestigationResult> {
    if (!this.env.TINYFISH_API_KEY) {
      throw new HttpError(500, "TINYFISH_MODE is set to sdk, but TINYFISH_API_KEY is missing.");
    }
    ...
    const repoPage = await this.fetchTinyFishSurface(repoData.html_url).catch((error: Error) => {
      partialFailures.push(`GitHub repository page inspection failed: ${error.message}`);
      return null;
    });
    ...
    const repoSurface = pushSurface({
      ...
      status: "ok",
      title: repoPage?.title ?? repoData.full_name,
      textContent: clip(
        [
          repoData.description ?? "",
          repoData.topics?.length ? `Topics: ${repoData.topics.join(", ")}` : "",
          languages.length ? `Languages: ${languages.join(", ")}` : "",
          repoPage?.snippet ? `Page summary: ${repoPage.snippet}` : ""
        ]
          .filter(Boolean)
          .join(" | "),
        1200
      ),
      snippet: clip(repoPage?.snippet ?? repoData.description ?? "", 220),
      notes: repoPage?.headings.slice(0, 4) ?? []
    });
  ...
  private async inspectOptionalSurface(
    ...
    try {
      const page = await this.fetchTinyFishSurface(url);
      return pushSurface({
        surfaceType,
        label,
        url,
  ...
  private async fetchTinyFishSurface(url: string): Promise<FetchedWebSurface> {
    const baseUrl = this.env.TINYFISH_BASE_URL.replace(/\/$/, "");
    const response = await fetch(`${baseUrl}/automation/run`, {
      ...
      body: JSON.stringify({
        ...
        goal:
          "Inspect this public page and return the page title, the most important visible body content, and a short array of headings in JSON."
      })
    });

    if (!response.ok) {
      const payload = (await response.json().catch(() => null)) as TinyFishAutomationResponse | null;
      const message = payload?.error?.message ?? `TinyFish request failed with status ${response.status}.`;
      throw new HttpError(response.status, message);
    }

    const payload = (await response.json()) as TinyFishAutomationResponse;
    return extractTinyFishPage(url, payload);
  }
}
